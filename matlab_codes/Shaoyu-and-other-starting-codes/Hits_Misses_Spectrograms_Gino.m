function Hits_Misses_Spectrograms_Gino(data,hitIndx,missIndx,bn,tapers,fs,dn,fk,pad,nPerm,alpha,iSubject,iSess,iCh)
...
[HitSpec,fhit] = tfspec(data(hitIndx,:),tapers,fs,dn,fk,pad,0.05,0,1); % get the time-frequency spectrum of the hit trial. Format trial x time x freq
[MissSpec,fmiss] = tfspec(data(missIndx,:),tapers,fs,dn,fk,pad,0.05,0,1); % get the time-frequency spectrum of the miss trial. Format trial x time x freq 

%% make directories for data and figures 

display('Creating directories...')
% Data directories %%%%%%%%%%%%%%%%%%%%%%%%%%%
Dir_dataHits = sprintf('Data/Hits/%d_Subject/%d_Sess/%d_Ch/%d_Subject_%d_Sess_%d_Ch',iSubject,iSess,iCh,iSubject,iSess,iCh)
if ~exist(Dir_dataHits, 'dir')
    mkdir(Dir_dataHits)
end

Dir_dataMisses = sprintf('Data/Misses/%d_Subject/%d_Sess/%d_Ch/%d_Subject_%d_Sess_%d_Ch',iSubject,iSess,iCh,iSubject,iSess,iCh)
