function plotFieldzScoreBipolarSpectrogramGino(data,hitIndx,missIndx,bn,tapers,fs,dn,fk,pad,nPerm,alpha)

Spec = []; TestSpec = []; DiffSpec = []; AvDiffSpec = [];
[Spec,f] = tfspec(data(hitIndx,:),tapers,fs,dn,fk,pad); % get the time-frequency spectrum of the hit trial
nWin = size(Spec,2);
