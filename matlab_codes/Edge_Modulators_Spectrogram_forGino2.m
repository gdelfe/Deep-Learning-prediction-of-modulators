...
for iSess = 2 %UsedSess Sessions with no modulators: iSess = 1,3,4,5, 10
...
%                 dlmwrite(sprintf('recorded_pairs_modulators_Sess_%d.txt',iSess),Data.RecordPair,'delimiter',',');
                ...
                if ~isempty(Data.Spec.ROC.sigChs{iFreqBand})
                    ...
                    for iCh = 34 %:2
                        ...
                        time = 1000              
                        figure;
                        plot(ModulatorLFP_Pre(1,1:time));
                        ylim([-200,200])
                        hold on
                        plot(SenderLFP_Pre(1,1:time));
                        hold on
                        plot(ReceiverLFP_Pre(1,1:time));
...
%                         dn = tapers(1)/15
                        ...
                        figure; tvimage(sq(log(spec_H(indx,:,:)))); title(sprintf('Hit - N = %.2f, W = %d, dn = %.4f, k = %d',tapers(1),tapers(2),dn,k)); colorbar;
                        
                        N = tapers(1);
                        nt = size(ModulatorLFP_Pre(hitIndx,:),2);
                        nwin = single(floor((nt-N*1000)./(dn*1000)))
                        ts = linspace(N/2,nt/1000-N/2,nwin)
                    
                        
%                         spec_M = tfspec_GINO(ModulatorLFP_Pre(missIndx,:),tapers,fs,dn,fk,pad,0.05,0,1);
%                         figure; tvimage(sq(log(spec_M(indx,:,:)))); title(sprintf('Miss - N = %.2f, W = %d, dn = %.4f, k = %d',tapers(1),tapers(2),dn,k)); colorbar;
%                         size(spec_H)
%                                                                 
%                         % Complex analysis %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
