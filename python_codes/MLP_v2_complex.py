...
tensor_hit_train = tensor_hit_trainA[:,tmin:tmax,fmin:fmax].copy()
tensor_hit_test = tensor_hit_testA[:,tmin:tmax,fmin:fmax].copy()
tensor_miss_train = tensor_miss_trainA[:,tmin:tmax,fmin:fmax].copy()
tensor_miss_test = tensor_miss_testA[:,tmin:tmax,fmin:fmax].copy()


plt.imshow(tensor_miss_trainA[0,:,:,0].transpose(),origin='lower')
plt.show()
plt.imshow(tensor_miss_trainA[0,:,:,1].transpose(),origin='lower')
plt.show()
...
for z in range(0,2): # for both the real and imaginary dimension
    for indx in range(tensor_train.shape[0]):
        mean = np.mean(tensor_train[indx,:,:,z])
        std = np.std(tensor_train[indx,:,:,z])
        tensor_train[indx,:,:,z] = (tensor_train[indx,:,:,z] - mean)/std
        
    for indx in range(tensor_test.shape[0]):
        mean = np.mean(tensor_test[indx,:,:,z])
        std = np.std(tensor_test[indx,:,:,z])
        tensor_test[indx,:,:,z] = (tensor_test[indx,:,:,z] - mean)/std
        

# plot the real part normalized
img = plt.imshow(tensor_train[0,:,:,0].transpose(),origin='lower')
plt.colorbar(img,orientation="horizontal")
plt.show()
# plot the imaginary part normalized
plt.imshow(tensor_train[0,:,:,1].transpose(),origin='lower')
plt.colorbar(img,orientation="horizontal")
plt.show()
...
model = ANN.Linear_Regression(tensor_train.shape[1],tensor_train.shape[2],2)
print(model)
...
model = ANN.MLP(tensor_train.shape[1],tensor_train.shape[2],2,0.2)
print(model)

#%%
# ### Multilayer perceptron (with dropout)

model = ANN.MLP_deep(tensor_train.shape[1],tensor_train.shape[2],2,0.2)
print(model)
