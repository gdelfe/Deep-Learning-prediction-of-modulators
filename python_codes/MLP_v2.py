...
tensor_miss_trainA = tensor_miss_trainA[1:].copy()
tensor_miss_testA = tensor_miss_testA[1:].copy()

tensor_hit_trainA = tensor_hit_trainA[1:].copy()
tensor_hit_testA = tensor_hit_testA[1:].copy()

# ### Balance the data set
tensor_hit_trainA = tensor_hit_trainA[0:tensor_miss_trainA.shape[0]+1]
tensor_hit_testA = tensor_hit_testA[0:tensor_miss_testA.shape[0]+1]
...
#%%
tensor_miss_train = tensor_miss_trainA[:,tmin:tmax,fmin:fmax]
tensor_miss_test = tensor_miss_testA[:,tmin:tmax,fmin:fmax]

tensor_hit_train = tensor_hit_trainA[:,tmin:tmax,fmin:fmax]
tensor_hit_test = tensor_hit_testA[:,tmin:tmax,fmin:fmax]


#%%
plt.imshow(tensor_hit_trainA[0].transpose(),origin='lower')
plt.show()
plt.imshow(tensor_hit_trainA[0,tmin:tmax,fmin:fmax].transpose(),origin='lower')
plt.show()
...
plt.imshow(tensor_test[0,:,:].transpose(),origin='lower')
...
model = ANN.MLP_deep(tensor_hit_train.shape[1],tensor_hit_train.shape[2],0.2)
print(model)
...
directory = '/all_pulses_N_015_W_25_dn_018'
dir_frequency = '/frequency_0_60'
# dir_tapers = '/tapers_3_4'
parameters = 'Ch41, 1st pulse only, N=0.15, W=25, dn=0.18, size=46*30, f=[0,60]Hz'
filename_parameters = '1Ch_1st_pulse_only_f_{}_{}_t_all_size_46x30.png'.format(fmin,fmax)
...
if NN == 'LR':
    fig.savefig('Ryan_data/Plots/Ch_{}'.format(Ch) + dir_frequency + directory + '/LR_{}'.format(filename_parameters),bbox_inches='tight',bbox_extra_artists=[my_suptitle])
elif NN == 'MLP':
    fig.savefig('Ryan_data/Plots/Ch_{}'.format(Ch) + dir_frequency + directory + '/MLP_{}'.format(filename_parameters),bbox_inches='tight',bbox_extra_artists=[my_suptitle])
elif NN == 'MLPd':
    fig.savefig('Ryan_data/Plots/Ch_{}'.format(Ch) + dir_frequency + directory + '/MLP_deeper_{}'.format(filename_parameters),bbox_inches='tight',bbox_extra_artists=[my_suptitle])
elif NN == 'CNN':
    fig.savefig('Ryan_data/Plots/Ch_{}'.format(Ch) + dir_frequency + directory + CNN_filters +filename_parameters,bbox_inches='tight',bbox_extra_artists=[my_suptitle])
elif NN == 'CNNd':
    fig.savefig('Ryan_data/Plots/Ch_{}'.format(Ch) + dir_frequency + directory + CNN_filters +filename_parameters,bbox_inches='tight',bbox_extra_artists=[my_suptitle])
